...
export async function getLineClusteredColumnComboChartInfo(visual: any) {
  ...
  if (columnData.length != 0) {
    ...
    if (axis) {
      barInfo += "The bars are separated horizontally by " + axis + ".<br>";
    }
    if (columnData.length == 1) {
      barInfo +=
        "The bars of this chart represents " + columnData[0].attribute + ".";
    } else {
      ...
      barInfo +=
        "Each " +
        axis +
        " has more than one bar. The bars of this chart represent " +
        dataString +
  ...
  if (lineData.length != 0) {
    ...
    if (axis) {
      lineInfo +=
        "The lines show the development of their data over the " +
        axis +
        ".<br>";
    }
    if (lineData.length == 1) {
      lineInfo +=
        "The line represents the data of " + lineData[0].attribute + ".<br>";
    } else {
      const lineAttributs = lineData.map((yAxis) => yAxis.attribute);
      const dataString = helpers.dataToString(lineAttributs);
      lineInfo +=
        "This chart has more than one line. The lines represent " +
        dataString +
        ". They are distinguishable by their color.<br>";
    }
  ...
  if (CGVisual?.encoding.yAxes[0].isVisible) {
    const dataAttributs = CGVisual?.encoding.yAxes.map(
      (yAxis) => yAxis.attribute
    );
    const dataString = helpers.dataToString(dataAttributs);
    generalImages.push("yAxisImg");
    generalInfos.push(
      "The Y-axes display the values of the " + dataString + "."
    );
  }
  if (await helpers.isVisible(visual, "legend")) {
    generalImages.push("legendImg");
    generalInfos.push(
      "The legend displays, which data is represented by a " +
        CGVisual?.mark +
        " of this chart. It also shows the corresponding color."
    );
    interactionImages.push("legendClickImg");
    interactionInfos.push(
      "When clicking on one of the labels in the legend you can filter the report by this data."
    );
  }
