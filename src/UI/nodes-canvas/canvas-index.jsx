...
function NodesCanvas() {
  ...
  const onEdgeUpdate = useCallback((oldEdge, newConnection) => {
    ...
    //reset the color
    let nodeTarget = document.querySelector(
      '[data-id="' + oldEdge.target + '"]'
    );
    let inputHandle = nodeTarget.querySelector(
      '[data-handleid="' + oldEdge.targetHandle + '"]'
    );
    inputHandle.classList.add("empty");
    //if the new connection is not empty
    nodeTarget = document.querySelector(
      '[data-id="' + newConnection.target + '"]'
    );
    inputHandle = nodeTarget.querySelector(
      '[data-handleid="' + newConnection.targetHandle + '"]'
    );
    ...
    //get path color
    let nodeSource = document.querySelector(
      '[data-id="' + newConnection.source + '"]'
    );
    let outputHandle = nodeSource.querySelector(
      '[data-handleid="' + newConnection.sourceHandle + '"]'
    );
  ...
  const onConnect = useCallback((params) => {
    const nodeSource = document.querySelector(
      '[data-id="' + params.source + '"]'
    );
    const nodeTarget = document.querySelector(
      '[data-id="' + params.target + '"]'
    );
    let outputHandle = nodeSource.querySelector(
      '[data-handleid="' + params.sourceHandle + '"]'
    );
    let inputHandle = nodeTarget.querySelector(
      '[data-handleid="' + params.targetHandle + '"]'
    );
    inputHandle.style.background = outputHandle.style.background;
