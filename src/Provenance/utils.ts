 ...
 */
export async function getCurrentVisuals(
  report: Report
): Promise<VisualDescriptor[]> {
  try {
    const vd: VisualDescriptor[] = await report.getPages().then(async (pages) =>
      pages
        .filter((page) => page.isActive)[0]
        .getVisuals()
        .then((visuals) =>
          visuals.filter((v) => v.type !== "shape" && v.type !== "textbox")
        )
    );
    return vd;
  } catch (err) {
