...
export async function setProvenanceTraversalStrategy(prov: any) {
  ...
  const sorted = Object.keys(counts)
    .sort(_sort)
    .filter(
      (i) => i !== "Page Reloaded" && i !== "Root" && i !== "Data Changed"
    );
  const charts_for_traversal = [];

  for (const k in sorted) {
    const obj = Object.keys(vis)
      .filter((key) => key === sorted[k])
      .reduce((cur, key) => {
        return Object.assign(cur, { [key]: vis[key] });
      }, {});
...
export async function setProvenanceTraversalStrategyLLM(prov: any) {
  ...
  for (const val of data.order) {
    const traversal_visual = visuals.filter(
      (v) => v.type === val.type && v.title == val.title
    )[0];
